...
namespace Sketchfab
{
    internal class SketchfabImporterWindow : EditorWindow
    {
        ...
        private readonly string _sourceFileHint = "Select or drag and drop a file";
        ...
        private readonly string[] fileFilters = {"GLTF Model", "gltf,glb", "Archive", "zip"};
        private readonly float minWidthButton = 150;

        // UI elements
        private readonly Vector2 UI_SIZE = new(350, 21);
        ...
        {
            var window = (SketchfabImporterWindow) GetWindow(typeof(SketchfabImporterWindow));
        ...
        private void OnCheckVersionFailure()
        {
        ...
        {
            var gltfFile = "";
            var info = new DirectoryInfo(_unzipDirectory);
            foreach (var fileInfo in info.GetFiles())
            {
                _unzippedFiles.Add(fileInfo.FullName);
                if (Path.GetExtension(fileInfo.FullName) == ".gltf") gltfFile = fileInfo.FullName;
            }
        ...
        private void checkValidity()
        {
            ...
            if (_ui == null) _ui = new SketchfabUI();
            if (_importer == null) Initialize();
        }
        ...
        {
            var modelfileName = Path.GetFileNameWithoutExtension(_importFilePath);
        ...
        private void displayInputInfos()
        {
            ...
            {
                var filepath = EditorUtility.OpenFilePanelWithFilters("Choose a file to import",
                    GLTFUtils.getPathAbsoluteFromProject(_importDirectory), fileFilters);
        ...
        private void displayImportDirectory()
        {
            ...
            GUILayout.BeginHorizontal();
            GUILayout.TextField(GLTFUtils.getPathProjectFromAbsolute(_importDirectory), GUILayout.MinWidth(UI_SIZE.x),
                GUILayout.Height(UI_SIZE.y));
            GUILayout.FlexibleSpace();
            if (GUILayout.Button("Change destination", GUILayout.Height(UI_SIZE.y), GUILayout.Width(minWidthButton)))
            {
                var newImportDir = EditorUtility.OpenFolderPanel("Choose import directory", _importDirectory, "");
        ...
        private void displayImportOptions()
        {
            ...
            _currentSampleName =
                GUILayout.TextField(_currentSampleName, GUILayout.MinWidth(200), GUILayout.Height(UI_SIZE.y));
            GUILayout.FlexibleSpace();
            GUILayout.EndHorizontal();
        ...
        private void displayImportButton()
        {
            ...
            if (GUILayout.Button("IMPORT", _ui.getSketchfabButton())) processImportButton();
            GUI.color = old;
            GUI.enabled = true;
            GUILayout.EndHorizontal();
        }
        ...
        {
            for (var i = 0; i < nbLines; ++i) GUILayout.Label("");
        }

        private void changeDirectory()
        {
            _importDirectory =
                EditorUtility.OpenFolderPanel("Choose import directory in Project", Application.dataPath, "Assets");
